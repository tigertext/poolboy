...
init([], _WorkerArgs, #state{size = Size, supervisor = Sup} = State) ->
    ...
    lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, new_worker when init, length ~p, workers ~p", [get_cluster_name(), length(Workers), Workers]),
...
handle_cast({cancel_waiting, CRef}, State) ->
    case ets:match(State#state.monitors, {'$1', CRef, '$2'}) of
        [[Pid, MRef]] ->
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, cancel_waiting, pid ~p, overflow ~p", [get_cluster_name(), Pid, State#state.overflow]),
        ...
        [] ->
            ...
            Waiting =/= State#state.waiting andalso
                lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, cancel_waiting, waiting length ~p", [get_cluster_name(), queue:len(Waiting)]),
...
handle_call({checkout, CRef, Block}, {FromPid, _} = From, State) ->
    ...
    case Workers of
        ...
        [] when MaxOverflow > 0, Overflow < MaxOverflow ->
            ...
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, checkout, no available worker, new_worker ~p overflow ~p", [get_cluster_name(), Pid, State1#state.overflow]),
            {reply, Pid, State1};
        [] when Block =:= false ->
            {reply, full, State};
        [] ->
            ...
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, checkout, no available worker, no overflow, overflow ~p, waiting length ~p", [get_cluster_name(), State#state.overflow, queue:len(Waiting)]),
            {noreply, State#state{waiting = Waiting}}
    end;
...
handle_info({'DOWN', MRef, Type, Object, Info}, State) ->
    case ets:match(State#state.monitors, {'$1', '_', MRef}) of
        [[Pid]] ->
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handled calling process down, worker pid ~p, overflow ~p, type ~p, object ~p, info ~p", [get_cluster_name(), Pid, State#state.overflow, Type, Object, Info]),
            true = ets:delete(State#state.monitors, Pid),
            NewState = handle_checkin(Pid, State),
            {noreply, NewState};
        [] ->
            ...
            Waiting =/= State#state.waiting andalso
                lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, waiting calling process down, overflow ~p, waiting length ~p, type ~p, object ~p, info ~p", [get_cluster_name(), State#state.overflow, queue:len(Waiting), Type, Object, Info]),
            {noreply, State#state{waiting = Waiting}}
    end;
handle_info({'EXIT', Pid, _Reason}, State) ->
    ...
    case ets:lookup(Monitors, Pid) of
        [{Pid, _, MRef}] ->
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, working worker down, delete pid ~p, overflow ~p, available worker count ~p", [get_cluster_name(), Pid, State#state.overflow, length(State#state.workers)]),
            true = erlang:demonitor(MRef),
            true = ets:delete(Monitors, Pid),
        ...
        [] ->
            case lists:member(Pid, State#state.workers) of
                true ->
                    ...
                    lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, available worker down, new_worker ~p, overflow ~p, available worker count ~p", [get_cluster_name(), NewWorker, State#state.overflow, length(State#state.workers)]),
...
handle_checkin(Pid, State) ->
    ...
    case queue:out(Waiting) of
        {{value, {From, CRef, MRef}}, Left} ->
            ...
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handle_checkin - handle waiting, pid ~p, overflow ~p, available worker count ~p, waiting length ~p", [get_cluster_name(), Pid, State#state.overflow, length(State#state.workers), queue:len(Left)]),
            State#state{waiting = Left};
        {empty, Empty} when Overflow > 0 ->
            ok = dismiss_worker(Sup, Pid),
            State1 = State#state{waiting = Empty, overflow = Overflow - 1},
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handle checkin - no waiting, dismiss_worker pid ~p, overflow ~p, available worker count ~p", [get_cluster_name(), Pid, State1#state.overflow, length(State#state.workers)]),
            State1;
        {empty, Empty} ->
...
handle_worker_exit(Pid, State) ->
    ...
    case queue:out(State#state.waiting) of
        {{value, {From, CRef, MRef}}, LeftWaiting} ->
            ...
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handle_worker_exit - handle waiting, new_worker pid ~p, overflow ~p, available worker count ~p, waiting length ~p", [get_cluster_name(), NewWorker, State#state.overflow, length(State#state.workers), queue:len(LeftWaiting)]),
            State#state{waiting = LeftWaiting};
        {empty, Empty} when Overflow > 0 ->
            State1 = State#state{overflow = Overflow - 1, waiting = Empty},
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handle_worker_exit - no waiting, decrease overflow, overflow ~p, available worker count ~p", [get_cluster_name(), State1#state.overflow, length(State#state.workers)]),
            State1;
        {empty, Empty} ->
            NewWorker = new_worker(Sup),
            Workers =
            ...
            lager:info(?RESOURCE_QUEUE_REDESIGN_LOG_PREFIX ++ "pool name ~p, handle_worker_exit - no waiting, no overflow, new_worker pid ~p, overflow ~p, available worker count ~p", [get_cluster_name(), NewWorker, State#state.overflow, length(State#state.workers)]),
